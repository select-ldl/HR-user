...
def wifi_password_crack(wifi_name):
    ...
    wifi_dic_path=os.path.abspath('pwd.txt')
        ...
        for pwd in f:
            ...
            while time.time() - start_time < 1.5:
                ...
                if interface.status() == 4:
                    ...
                    print('\r连接成功！密码为：{%s}'%pwd)
                    exit(0)
                else:
                    # print(f'\r正在利用密码 {pwd} 尝试破解。', end='')
                    print('\r正在利用密码 {%s} 尝试破解。'%pwd, end='')
                    ...
                    if target_num in range(len(wifi_list)):
                        ...
                        while not choose_exit_flag:
                            try:
                                ...
                                choose = str(input('你选择要破解的WiFi名称是：{%s}，确定吗？（Y/N）'%wifi_list[target_num][1]))
                                # 对用户输入进行小写处理，并判断
                                if choose.lower() == 'y':
                                    choose_exit_flag = 1
                                elif choose.lower() == 'n':
